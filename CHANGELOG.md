...
- internal 路由上游错误日志：代理飞书 OpenAPI 的 internal 资源路由在上游返回 >=400 时，把状态码与响应体片段（脱敏、截断 500 字符）写入 warn 日志。此前下游只能拿到状态码（如表情包资源 500），生产排障无法定位真实原因。

## 0.4.1

### Fixed

- 思考期 reaction 仅私聊或群聊中被 @ 时触发，群聊中不 @ 机器人不再贴表情（含诊断日志辅助定位运行时数据 shape）。
